...
public abstract class TupleInputFormat<T extends Tuple> extends FileInputFormat<T> {
	/**
	 * The log.
	 */
	...
	protected class BlockBasedInput extends FilterInputStream {
		...
		public BlockBasedInput(FSDataInputStream in, int blockSize) {
			super(in);
			this.blockPos = (int) TupleInputFormat.this.blockInfo.getFirstRecordStart();
			this.maxPayloadSize = blockSize - TupleInputFormat.this.blockInfo.getInfoSize();
		}
		...
		private void skipHeader() throws IOException {
			byte[] dummy = new byte[TupleInputFormat.this.blockInfo.getInfoSize()];
